...
function S_UPGRADE (data) {
	...
	dbBase.query(stringQuery,function (error,rows) {
		if (!!error){DetailError = ('Upgrade.js: query '+stringQuery); functions.WriteLogError(DetailError);}
...
function materialCalc (dbBase,data,rowUpgrade) {
	...
	db_upgrade_database.query(stringMaterial,function (error,rows) {
		if (!!error){DetailError = ('Upgrade.js: materialCalc' + data.ID_User);functions.WriteLogError(DetailError);}
...
function checkMaterial (dbBase,data,rowsMaterial) {
	...
	dbBase.query(stringQuery,function (error,rows) {
		if (!!error){DetailError = ('Upgrade.js: query checkMaterial : '+ data.ID_User); functions.WriteLogError(DetailError);}
...
function updateUserMaterial (dbBase,data,rowsMaterial) {
	...
	dbBase.query(stringUpdateUser,function (error,result) {
		if (!!error){DetailError = ('Upgrade.js: updateUserMaterial : '+ data.ID_User); functions.WriteLogError(DetailError);}
		LogChange='Upgrade.js: updateUserMaterial: '+stringUpdateUser;functions.LogChange(LogChange);	
	});
}
...
function updateBaseUser (dbBase,data,rowUpgrade) {
	...
	dbBase.query(stringUpgrade,function (error,result) {
		if (!!error){DetailError = ('Upgrade.js: updateBaseUser: '+ data.ID_User); functions.WriteLogError(DetailError);}
		LogChange='Upgrade.js: updateBaseUser: '+stringUpgrade;functions.LogChange(LogChange);	
...
function setTimerUpdateDatabase (timeOut,data) {
	...
	DictTimeOut[stringTimeOut] = setTimeout(function (data) {
		...
		db_all_user.query(queryUser,function (error,rows) {
			if (!!error){DetailError = ('Upgrade.js: query getData : '+ rowsData.ID_User); functions.WriteLogError(DetailError);}
			...
			if (rows!=undefined) {
				...
				dbUpgrade.query(stringUpdateBaseUpgrade,function (error,result) {
					...
					LogChange='Upgrade.js: updateLevel: '+stringUpdateBaseUpgrade;functions.LogChange(LogChange);
				});
				...
				db_all_user.query(stringUpdate_Game_info,function (error,result_stringUpdate_Game_info) {
					...
					LogChange='Upgrade.js: updateMight: '+stringQueryMightBonus;functions.LogChange(LogChange);
				});
			});
...
function checkUnlock (dbUpgrade,data) {
	...
	db_upgrade_database.query(stringQuery,function(error,rows){
		if (!!error){DetailError = ('Upgrade.js: query checkUnlock : '+ data); functions.WriteLogError(DetailError);}
		...
		db_upgrade_database.query(tableQuery,function(error,rows_tableQuery){
			...
			if (rows_tableQuery[0].Unlock_ID!=0) { returnUnlockID=rows_tableQuery[0].Unlock_ID;
				...
				dbUpgrade.query(stringUpgrade,function (error,result) {
					...
					if (!!error){DetailError = ('Upgrade.js: query checkUnlock upgrade: '+ data); functions.WriteLogError(DetailError);}
