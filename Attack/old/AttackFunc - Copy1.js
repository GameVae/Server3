...
function stopIntervalAttack (ID_User_Defend) {
	...
	new Promise((resolve,reject)=>{
		...
		client.hget(stringHUnit,stringKey,function (error,rows) {
			...
			unitResult.AttackedBool = 0;
			functions.ShowLog(functions.ShowLogBool.On,'AttackFunc.js stopIntervalAttack hset unitResult',[unitResult]);
	...
	new Promise((resolve,reject)=>{
		...
		client.hget(stringHPos,pos,function(error,rows){
			...
			if (listIDUnitAttack.length==0) {
				functions.ShowLog(functions.ShowLogBool.On,'Moving_Attack.js checkCurrentPos stop attack',[stringKey]);
					...
					if (resultUnitAttack!=null) {
						...
						for (var i = 0; i < resultUnitAttack.length; i++) {
							if (resultUnitAttack[i]!=null) {
								...
								functions.ShowLog(functions.ShowLogBool.On,'AttackFunc.js clearInterAttackUpdate=>updateRedis stringHUnit,unitID,stringUnitResult',[stringHUnit,unitID,stringUnitResult]);
							...
							if (i == (resultUnitAttack.length-1)) {
								functions.ShowLog(functions.ShowLogBool.On,'AttackFunc.js clearInterAttackUpdate =>resolve() resultUnitAttack.length',[resultUnitAttack.length,i]);
								resolve();
							}					
						}					
					}				
