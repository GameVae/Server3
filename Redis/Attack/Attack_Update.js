...
var attackFunc 				= require('./Attack.js')
...
exports.UpdateDatabase = function updateDatabase (Server_ID) {
	updateDatabase2 (Server_ID)
}

// updateDatabase2 (1)
function updateDatabase2 (Server_ID) {
	var stringHAttack = "s"+Server_ID+"_attack";
	client.hkeys(stringHAttack,function (error,rows) {
		if (rows.length>0) {
			for (var i = 0; i < rows.length; i++) {
				attackFunc.AttackInterval(Server_ID,rows[i]);
			}
		}
	});
}
